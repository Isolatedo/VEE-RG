...
class AnatomicalDetector(nn.Module):
    ...
    def forward(self, images: Tensor, targets: Optional[List[Dict[str, Tensor]]] = None):
        ...
        if self.return_feature_vectors:
            if self.training:
                ...
                output = losses, detections, top_region_features, class_detected
                if self.return_spatial_features:
                    return *output, spatial_features
                return output

    @torch.no_grad()
    def forward_spatial(self, images: Tensor) -> Dict[str, Tensor]:
        """Return the frozen detector features consumed by the VEE modules."""
        if self.training:
            raise RuntimeError("forward_spatial requires the detector in eval mode")
        if images.ndim != 4 or images.shape[1:] != (1, 512, 512):
            raise ValueError("images must have shape [B, 1, 512, 512]")
        output = self(images)
        if len(output) != 5:
            raise ValueError(
                "AnatomicalDetector must be built with feature and spatial outputs"
            )
        _, detections, roi, seen, c5 = output
        if roi.shape[1:] != (29, 1024):
            raise ValueError("detector ROI output has the wrong shape")
        if c5.ndim != 4 or c5.shape[1] != 2048:
            raise ValueError("detector C5 output has the wrong shape")
        return {
            "roi": roi,
            "seen": seen.bool(),
            "c5": c5,
            "boxes": detections["top_region_boxes"],
        }
